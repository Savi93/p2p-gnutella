...
def getGlobalIP():
    with urllib.request.urlopen("http://api.ipify.org/?format=json") as url:
    ...
    while close == 0:
        try:
            ...
            if(message["Command"] == 1): #PING
                ...
                if(isPrivateIP(message["IP"])):
                    data = json.dumps({"Command": 4, "Contents":arr, "IP":socket.gethostbyname(host)}) #Send QueryHit JSON file
                else:
                    data = json.dumps({"Command": 4, "Contents":arr, "IP":getGlobalIP()}) #Send QueryHit JSON file
...
def query():
    if(input("Do you want to continue? Type Y for YES\n") == "Y"):
        ...
        dataloc = json.dumps({"Command": 3, "Contents":song, "IP": socket.gethostbyname(socket.gethostname())})
        dataglob = json.dumps({"Command": 3, "Contents":song, "IP": getGlobalIP()})
        ...
        for peer in peers:
            if(isPrivateIP(peer)):
                udpsocket.sendto(dataloc.encode(), (peer, udpport))
            else:
                udpsocket.sendto(dataglob.encode(), (peer, udpport))
    ...
    while close == 0:
        ...
        for peer in peers:
            if(isPrivateIP(peer)):
                ...
                data = json.dumps({"Command": 1, "IP": getGlobalIP(), "TTL":1}) #Send PING JSON file
...
def pong(ip):
    if(isPrivateIP(ip)):
        ...
        data = json.dumps({"Command":2, "IP": getGlobalIP()})
